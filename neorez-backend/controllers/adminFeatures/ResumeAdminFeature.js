const ResumeBuilder = require('../../models/adminFeatures/ResumeAdminFeature');
...
exports.getResumeById = async (req, res) => {
    try {
        ...
        const resume = await ResumeBuilder.findById(id); // Find the document by _id
...
exports.updateResume = async (req, res) => {
    try {
        const { id } = req.params;
        const updatedResume = await ResumeBuilder.findByIdAndUpdate(id, req.body, { new: true });
        if (!updatedResume) {
            return res.status(404).json({ error: 'Resume not found' });
        }
...
exports.createOrUpdateResumeWithImage = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (id) {
            ...
            await newResume.save(); // Save the new resume in the database
            return res.status(201).json(newResume); // Return the created resume
        }
    } catch (error) {
